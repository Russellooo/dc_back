...
public class UserServiceImpl implements UserService {
	...
	@Override
	public JSONObject checkTime(JSONObject requestJson) {
		String orderTime = requestJson.getString("orderTime");
		List<Integer> sendTimes = (List<Integer>) requestJson.get("sendTime");
		String[] checkTime = StringUtils.split(orderTime, " ");
		String[] time = checkTime[1].split(":");
		Integer hour = Integer.valueOf(time[0]);
		Integer min = Integer.valueOf(time[1]);
		Integer check=2*hour;
		if(min>30){
			check+=1;
		}
		requestJson.put("inTime", sendTimes.contains(check));
		if(!sendTimes.contains(check)){
